...
class Zoom_Advanced(ttk.Frame):
    ...
    def selezione_in_corso(self,event):
        if self.radio_var.get()=="c" and self.is_inside_image(self.inizio_x,self.inizio_y):#se i punti iniziali fossero fuori l'area dell'immagine non farei nessun disegno
            if self.is_inside_image(event.x,event.y):
                ...
                self.canvas.delete("cerchio_temporaneo")
        if self.radio_var.get()=="r" and self.is_inside_image(self.inizio_x,self.inizio_y):
            if self.is_inside_image(event.x,event.y):
    ...
    def inizio_selezione(self, event):
            ...
            self.ultima_x = event.x
            self.ultima_y = event.y
        #print("inizio "+str(self.inizio_x)+","+str(self.inizio_y))
    ...
    def fine_selezione(self, event):
        ...
        if self.radio_var.get() == "c" and self.is_inside_image(event.x,event.y) and self.is_inside_image(self.inizio_x,self.inizio_y):#se non sono dentro l'immagine i vertici non creo nulla
            self.canvas.delete("cerchio_temporaneo")
        ...
        elif self.radio_var.get() == "r" and self.is_inside_image(event.x,event.y) and self.is_inside_image(self.inizio_x,self.inizio_y):
            self.canvas.delete("rettangolo_temporaneo")
    ...
    def reset_disegni(self,event=None):
        ...
        self.canvas.delete("rettangolo_temporaneo")
        self.canvas.delete("mano_libera")
